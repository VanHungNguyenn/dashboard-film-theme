...
import ActorData from 'assets/data/list/actor.data.json'
import {
	DeleteOutlined,
	SearchOutlined,
	PlusCircleOutlined,
...
const Actor = () => {
	const [list, setList] = useState(ActorData)
	const [selectedRows, setSelectedRows] = useState([])
	const [selectedRowKeys, setSelectedRowKeys] = useState([])

	const onSearch = (e) => {
		const value = e.currentTarget.value
		const searchArray = e.currentTarget.value ? list : ActorData
	...
	const handleShowType = (value) => {
		if (value !== 'GoogleIndex') {
			...
			const data = utils.filterArray(ActorData, key, value)
			setList(data)
		} else {
			setList(ActorData)
		}
