...
import ServerData from 'assets/data/list/server.data.json'
import {
	DeleteOutlined,
	SearchOutlined,
	PlusCircleOutlined,
...
const Server = () => {
	const [list, setList] = useState(ServerData)
	const [selectedRows, setSelectedRows] = useState([])
	const [selectedRowKeys, setSelectedRowKeys] = useState([])

	const onSearch = (e) => {
		const value = e.currentTarget.value
		const searchArray = e.currentTarget.value ? list : ServerData
	...
	const handleShowType = (value) => {
		if (value !== 'GoogleIndex') {
			...
			const data = utils.filterArray(ServerData, key, value)
			setList(data)
		} else {
			setList(ServerData)
		}
