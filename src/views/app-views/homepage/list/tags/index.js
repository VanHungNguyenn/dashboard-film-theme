...
import TagsData from 'assets/data/list/tags.data.json'
import {
	DeleteOutlined,
	SearchOutlined,
	PlusCircleOutlined,
...
const Tags = () => {
	const [list, setList] = useState(TagsData)
	const [selectedRows, setSelectedRows] = useState([])
	const [selectedRowKeys, setSelectedRowKeys] = useState([])

	const onSearch = (e) => {
		const value = e.currentTarget.value
		const searchArray = e.currentTarget.value ? list : TagsData
	...
	const handleShowType = (value) => {
		if (value !== 'GoogleIndex') {
			...
			const data = utils.filterArray(TagsData, key, value)
			setList(data)
		} else {
			setList(TagsData)
		}
